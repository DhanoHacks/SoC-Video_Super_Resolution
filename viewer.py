...
def view(scale, img_file, version, d):
    ...
    with torch.no_grad():
        ...
        write_image(f"outputs/Image SR/ver {version}/bicubic_x{scale}.png", bicubic_image)

        plt.subplot(1,3,2)
        plt.title("bicubic")
        plt.imshow(torch.permute(bicubic_image, (1,2,0)))
    ...
    write_image(f"outputs/Image SR/ver {version}/sr_x{scale}.png", sr_image)

    
    plt.subplot(1,3,1)
    plt.title("low res image")
    plt.imshow(torch.permute(lr_image, (1,2,0)))

    

    plt.subplot(1,3,3)
    plt.title("sr image")
    plt.imshow(torch.permute(sr_image, (1,2,0)))
...
view(args.scale, args.img_file, args.ver, args.device)